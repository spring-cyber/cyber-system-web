<template>
  <c-page-label title="应用管理" icon="cyber-yingyong" document-link="javascript:;">
  ...
  <c-radio-wrapper v-model:value="queryState.status" @change="methods.searchQuery">
    ...
    <c-radio-button
      v-for="(item, index) in $dictStore.status"
      :key="index"
      :value="item.value"
  ...
  >
    ...
    <template #bodyCell="{ column, record }">
      ...
      <template v-if="column.key == 'type'">
        <c-cell-dict :options="$dictStore.productType" :value="record.type"></c-cell-dict>
      </template>
...
const methods = {
  ...
  delete(record) {
    deleteConfrim({
      content: `是否确认删除“${record.name}（${record.code}）”的应用及其相关数据？`,
      value: record.code,
    }, {
      url: '/system/product',
      method: 'delete',
